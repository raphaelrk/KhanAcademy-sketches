// https://www.khanacademy.org/computer-programming/mandelbrot-visited-points/4695920118071296

var baseB = 20; //base brightness
var maxB = 500; // brightness of set
var maxIters = 60; // how many iterations done before stopping
...
var calcIters = function(origX, origY, lastX, lastY, thisIteration) {
    ...
    strokeWeight(0.1);
    stroke(0, 0, 0, 10);
    point(map(newX, -2, 2, 0, 400), map(newY, -2, 2, 0, 400));
...
var draw = function() {
    ...
    for(var r = 0; r < 400; r += blockSize) {
        ...
        if(antialias) {
            var x2 = map(c, 0, 400, lBound, rBound) + (rBound - lBound)/400;
            var y2 = map(r, 0, 400, tBound, bBound) + (bBound - tBound)/400;
