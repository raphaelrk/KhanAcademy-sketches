// https://www.khanacademy.org/computer-programming/winter-tree-fractal/2334537729
...
if(grass_bg) {
    for(var y = 1; y < 20; y+=random(1, 3)) {
        for(var x = 0; x < 400; x+=random(y/20, y/5)) {
            ...
            noStroke();
            fill(255, 255, 255);
            ellipse(x + slant, y1 - len, slant*4, slant);
        }
    }
}
...
if(grass_foreground) {
    for(var x = 0; x < 400; x+= random(1, 3)) {
        stroke(104, 204 + random(-50, 30), 159);
        strokeWeight(random(0.5, 2));
        line(x, 400, x + random(-4, 4), 400 - random(5, 30));
        noStroke();
        fill(255, 255, 255);
        ellipse(x, 400 - random(0, 25), 
                random(4,8), random(2,8));
    }
}
